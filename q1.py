...
def L2Loss(y:np.ndarray, y_hat:np.ndarray) -> float:
    ...
    '''
    # YOUR CODE HERE
    loss =(1/(2*y.shape[1]))*np.sum((y - y_hat)**2)
    # YOUR CODE ENDS HERE
    return loss
...
class FCNN:
    def __init__(self, input_size:int, hidden_size:int, output_size:int, init:str='random', AutograderSeed:int=0):
        ...
        self.numTrainableParams = self.W1.size + self.W2.size
    ...
    def forward(self, x:np.ndarray, train:bool=False) -> np.ndarray:
        ...
        forwardPassResults['z1'] = np.dot(self.W1, x)
        forwardPassResults['a1'] = sigmoid(forwardPassResults['z1'])
        forwardPassResults['z2'] = np.dot(self.W2, forwardPassResults['a1'])
    ...
    def backward(self, forwardPassResults, y_predicted:np.ndarray, y_true:np.ndarray) -> dict:
        ...
        N = y_true.shape[1]  # number of examples
        
        # Gradient for output layer (W2)
        dz2 = (y_predicted - y_true) * sigmoidDerivative(forwardPassResults['z2'])
        gradients['dW2'] = (1/N) * np.dot(dz2, forwardPassResults['a1'].T)
        
        # Gradient for hidden layer (W1)
        da1 = np.dot(self.W2.T, dz2)
        dz1 = da1 * sigmoidDerivative(forwardPassResults['z1'])
        gradients['dW1'] = (1/N) * np.dot(dz1, forwardPassResults['x'].T)
