...
def assignClusters(data, clusters):
    ...
    '''
    # YOUR CODE HERE
    num_points = data.shape[1]  # 150 data points
    num_clusters = clusters.shape[1]  # k clusters
    ...
    for i in range(num_points):
        ...
        for j in range(num_clusters):
            # Euclidean distance between data point i and cluster j
            distance = np.linalg.norm(data[:, i] - clusters[:, j])
...
def updateClusters(data, assignments, k):
    ...
    '''
    # YOUR CODE HERE
    clusters = np.zeros((2, k))  # Initialize 2xk array for k cluster centers
    
    for i in range(k):
        # Find all data points assigned to cluster i
        points_in_cluster = data[:, assignments == i]
        
        if points_in_cluster.shape[1] > 0:  # If cluster has points assigned
            # Compute mean of all points in this cluster
            clusters[:, i] = np.mean(points_in_cluster, axis=1)
        else:
            # If no points assigned, keep previous cluster center (or random)
            clusters[:, i] = data[:, np.random.randint(0, data.shape[1])]
